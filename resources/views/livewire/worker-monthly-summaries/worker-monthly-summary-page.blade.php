...
<div>
        ...
        <div class="p-4 border-b border-gray-200 dark:border-gray-700 space-y-3">
            <div class="flex flex-col lg:flex-row lg:items-center gap-3">
                ...
                <div class="flex items-center space-x-2">
                    @can('worker_monthly_summaries.create')
        ...
        @if (count($selected) > 0)
            <div class="px-4 py-3 bg-emerald-50 dark:bg-emerald-900/20 border-b border-emerald-200 dark:border-emerald-800">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-2">
                        @can('worker_monthly_summaries.delete')
            ...
            <table class="min-w-full border-separate border-spacing-0">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        ...
                        <th wire:click="sortBy('paid_amount')" class="sticky top-0 z-10 px-3 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 cursor-pointer whitespace-nowrap">{{ __('app.paid_amount') }} {{ $sortField === 'paid_amount' ? ($sortDirection === 'asc' ? '↑' : '↓') : '' }}</th>
                                    ...
                                    <div x-data="{ editing: false, val: '{{ $formattedVal }}' }" x-init="$watch('editing', value => { if (value) $nextTick(() => $refs.input.focus()) })" class="flex items-center justify-end gap-1">
                                        <template x-if="!editing">
                                            <div class="flex items-center gap-1">
                                                ...
                                                @can('worker_monthly_summaries.edit')
                ...
                @if ($rows->count() > 0)
                    <tfoot class="bg-emerald-50/80 dark:bg-emerald-900/20 border-t-2 border-emerald-200 dark:border-emerald-800">
                        <tr>
                            <td colspan="3" class="px-3 py-3 text-right text-xs font-bold text-emerald-700 dark:text-emerald-300 uppercase tracking-wider">{{ __('app.totals') }}</td>
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6 z-10 max-h-[90vh] overflow-y-auto">
                ...
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        @foreach ([
                            'formTotalAmount' => __('app.total_amount'),
                            'formPaidAmount' => __('app.paid_amount'),
                            'formTotalHours' => __('app.total_hours'),
                            'formPayrollAmount' => __('app.payroll_amount'),
                            'formAdvanceAmount' => __('app.advance_amount'),
                            'formCreditAmount' => __('app.credit_amount'),
                            'formTicketAmount' => __('app.ticket_amount'),
                        ] as $field => $label)
                            <div>
    ...
    {{-- Delete Modal --}}
    @if ($showDeleteModal)
        @php $deletingRow = $editingId ? \App\Models\WorkerMonthlySummary::with(['monthlyPeriod', 'worker'])->find($editingId) : null; @endphp
                    ...
                    <div>
                        ...
                        @if ($deletingRow)
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {{ $deletingRow->worker?->full_name ?? '' }} — {{ $deletingRow->monthlyPeriod?->period_code ?? '' }}
                            </p>
                        @endif
                    </div>
                </div>
    ...
    @if ($showImportModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center" x-data x-on:keydown.escape.window="$wire.set('showImportModal', false)">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6 z-10 max-h-[90vh] overflow-y-auto">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100">{{ __('app.import_worker_monthly_summaries') }}</h3>
                ...
                @if ($importStep === 1)
                    <div class="space-y-4">
                        <p class="text-sm text-gray-600 dark:text-gray-300">{{ __('app.upload_excel_file_worker_monthly_summaries') }}</p>
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-3 py-2">
                                                    <select wire:model="importColumnMap.{{ $idx }}" class="text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 py-1 pl-2 pr-6">
                                                        ...
                                                        <option value="total_amount">{{ __('app.total_amount') }}</option>
                                                        <option value="paid_amount">{{ __('app.paid_amount') }}</option>
                                                        <option value="total_hours">{{ __('app.total_hours') }}</option>
                                                        <option value="payroll_amount">{{ __('app.payroll_amount') }}</option>
                                                        <option value="advance_amount">{{ __('app.advance_amount') }}</option>
                                                        <option value="credit_amount">{{ __('app.credit_amount') }}</option>
                                                        <option value="ticket_amount">{{ __('app.ticket_amount') }}</option>
                                                    </select>
                                                </td>
