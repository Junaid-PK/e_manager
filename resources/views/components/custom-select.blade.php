...
<div class="relative"
    ...
    <div x-show="open"
        ...
        <div x-ref="listbox"
            ...
            @endif
            <template x-for="item in filtered" :key="'csel-' + (item && item.value ? item.value : Math.random())">
                <button type="button" role="option"
                        @click="item && item.value !== undefined ? pick(item.value) : null"
                        :class="(cursor > -1 && filtered[cursor] && item && filtered[cursor].value === item.value)
                            ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300'
                            : 'text-gray-900 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-700'"
                        class="block w-full shrink-0 text-left px-3 py-1.5 text-xs truncate"
                        x-text="item && item.label ? item.label : ''"></button>
            </template>
